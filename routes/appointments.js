  ...
  try {
    ...
    if (existingAppointment) {
      return res.status(409).json({
        success: false,
        message:
          "You already have a pending appointment with this doctor at this time.",
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to request appointment." });
  }
});
  ...
  try {
    ...
    if (!appointmentId) {
      return res.status(400).json({
        success: false,
        message:
          "Appointment ID is required. Please request an appointment first.",
      });
    }
...
router.get("/check/:appointmentId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to check status." });
  }
});
...
router.patch("/request/:appointmentId/accept", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Appointment accepted!", appointment });
  } catch (error) {
    console.error("Accept failed:", error);
    res
      .status(500)
      .json({ success: false, message: "Failed to accept appointment." });
  }
});
...
router.get("/check-restriction/:email", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to check restriction." });
  }
});

...
router.get("/analytics", async (req, res) => {
  try {
    ...
    if (previousMonthReviews > 0) {
      const growth =
        ((recentReviews - previousMonthReviews) / previousMonthReviews) * 100;
    ...
    res
      .status(500)
      .json({ success: false, message: "Internal Server Analytics Error" });
  }
});
...
router.get("/users", async (req, res) => {
  try {
      ...
      .aggregate([
        ...
        },
        {
          $addFields: {
            ...
            appointmentCount: {
              $ifNull: [{ $arrayElemAt: ["$appointmentStats.total", 0] }, 0],
            },
          },
        },
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to map user directory" });
  }
});
...
router.patch("/users/restrict/:id", async (req, res) => {
  try {
    ...
    if (!days || days <= 0)
      return res
        .status(400)
        .json({ success: false, message: "Invalid restriction duration." });
    ...
    res
      .status(200)
      .json({ success: true, status: "restricted", restrictedUntil });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error restricting user." });
  }
});
...
router.get("/doctors", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching practitioners." });
  }
});
    ...
    if (verified === true) {
      ...
      if (!application) {
        return res.status(404).json({
          success: false,
          message:
            "Pending application not found. It may have already been approved.",
        });
      }
    ...
    const enrichedAppointments = await Promise.all(
      appointments.map(async (appt) => {
        const patient = await db
          .collection("user")
          .findOne({ _id: toObjectId(appt.patientId) });
        const doctor = await db
          .collection("Doctor")
          .findOne({ _id: toObjectId(appt.doctorId) });
  ...
  try {
    ...
    const enrichedPayments = await Promise.all(
      payments.map(async (payment) => {
        const patient = await db
          .collection("user")
          .findOne({ _id: toObjectId(payment.patientId) });
        const doctor = await db
          .collection("Doctor")
          .findOne({ _id: toObjectId(payment.doctorId) });

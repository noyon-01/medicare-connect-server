  ...
  try {
    ...
    const enrichedPayments = payments.map((payment) => {
      const apt = appointments.find(
        (a) => a._id.toString() === payment.appointmentId.toString(),
      );
      return {
        ...payment,
...
router.get("/favorite-doctors/:patientEmail", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, doctors: doctorsWithRatings.slice(0, 5) });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch favorite doctors." });
  }
});
